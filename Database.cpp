...
namespace dtmdl
{
	...
	result<void, string> Database::SetRecordBaseType(Rec def, TypeReference const& type)
	{
		...
		/// ChangeLog add
		AddChangeLog(json{ {"action", "SetRecordBaseType"}, {"type", def->Name()}, {"basetype", ToJSON(type) }, {"previous", ToJSON(def->BaseType()) } });
	...
	result<void, string> Database::SetFieldType(Fld def, TypeReference const& type)
	{
		...
		/// ChangeLog add
		AddChangeLog(json{ {"action", "SetFieldType"}, {"record", def->ParentRecord->Name()}, {"field", def->Name}, {"type", ToJSON(type) }, {"previous", ToJSON(def->FieldType) } });
